...
// проверка наличия типа T в очереди D

template < typename T, typename D, bool C = is_empty_v < D > > struct Has {};

////////////////////////////////////////////////////////////////////////////////////////////////

// Частичная специализация для непустой очереди
template < typename T, typename D > struct Has < T, D, false >
{
    // Если первый элемент равен T, то true, иначе проверяем остальную часть очереди
...
//  ------------------------------------------------------------------------------
    ...
    static_assert(!has_v < long, Deque < int, double, char > >);
    
    static_assert(!has_v < int, Deque < > >);
    static_assert(!has_v < double, Deque < > >);
    
    static_assert(has_v < int, Deque < int > >);
    static_assert(!has_v < double, Deque < int > >);
    
    static_assert(has_v < int, Deque < int, double > >);
    
    static_assert(has_v < char, Deque < int, double, char > >);
    
    static_assert(has_v < double, Deque < int, double, char, float > >);
    
    static_assert(has_v < int, Deque < int, int, int > >);
    static_assert(has_v < double, Deque < int, double, int > >);
